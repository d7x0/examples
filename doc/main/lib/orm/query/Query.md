...
* Методы:
    * Собственные методы [84]:

        * [Метод __construct](#метод-__construct)
        * [Метод __call](#метод-__call)
        * [Метод getSelect](#метод-getSelect)
        * [Метод setSelect](#метод-setSelect)
        * [Метод addSelect](#метод-addSelect)
        * [Метод getFilter](#метод-getFilter)
        * [Метод setFilter](#метод-setFilter)
        * [Метод addFilter](#метод-addFilter)
        * [Метод getFilterHandler](#метод-getFilterHandler)
        * [Метод getGroup](#метод-getGroup)
        * [Метод setGroup](#метод-setGroup)
        * [Метод addGroup](#метод-addGroup)
        * [Метод getOrder](#метод-getOrder)
        * [Метод setOrder](#метод-setOrder)
        * [Метод addOrder](#метод-addOrder)
        * [Метод getLimit](#метод-getLimit)
        * [Метод setLimit](#метод-setLimit)
        * [Метод getOffset](#метод-getOffset)
        * [Метод setOffset](#метод-setOffset)
        * [Метод union](#метод-union)
        * [Метод unionAll](#метод-unionAll)
        * [Метод setUnionOrder](#метод-setUnionOrder)
        * [Метод addUnionOrder](#метод-addUnionOrder)
        * [Метод setUnionLimit](#метод-setUnionLimit)
        * [Метод setUnionOffset](#метод-setUnionOffset)
        * [Метод enableDataDoubling](#метод-enableDataDoubling)
        * [Метод disableDataDoubling](#метод-disableDataDoubling)
        * [Метод enablePrivateFields](#метод-enablePrivateFields)
        * [Метод disablePrivateFields](#метод-disablePrivateFields)
        * [Метод isPrivateFieldsEnabled](#метод-isPrivateFieldsEnabled)
        * [Метод isFieldPrivate](#метод-isFieldPrivate)
        * [Метод registerRuntimeField](#метод-registerRuntimeField)
        * [Метод setCustomBaseTableAlias](#метод-setCustomBaseTableAlias)
        * [Метод filter](#метод-filter)
        * [Метод expr](#метод-expr)
        * [Метод exec](#метод-exec)
        * [Метод fetch](#метод-fetch)
        * [Метод fetchAll](#метод-fetchAll)
        * [Метод fetchObject](#метод-fetchObject)
        * [Метод fetchCollection](#метод-fetchCollection)
        * [Метод addToSelectChain](#метод-addToSelectChain)
        * [Метод setFilterChains](#метод-setFilterChains)
        * [Метод setFilterHandlerChains](#метод-setFilterHandlerChains)
        * [Метод divideFilter](#метод-divideFilter)
        * [Метод divideFilterHandler](#метод-divideFilterHandler)
        * [Метод checkFilterAggregation](#метод-checkFilterAggregation)
        * [Метод checkFilterHandlerAggregation](#метод-checkFilterHandlerAggregation)
        * [Метод rewriteDataDoubling](#метод-rewriteDataDoubling)
        * [Метод addToGroupChain](#метод-addToGroupChain)
        * [Метод addToOrderChain](#метод-addToOrderChain)
        * [Метод buildJoinMap](#метод-buildJoinMap)
        * [Метод buildJoin](#метод-buildJoin)
        * [Метод buildWhere](#метод-buildWhere)
        * [Метод buildGroup](#метод-buildGroup)
        * [Метод buildHaving](#метод-buildHaving)
        * [Метод buildOrder](#метод-buildOrder)
        * [Метод buildQuery](#метод-buildQuery)
        * [Метод getFilterCswFields](#метод-getFilterCswFields)
        * [Метод prepareJoinReference](#метод-prepareJoinReference)
        * [Метод prepareJoinFilterReference](#метод-prepareJoinFilterReference)
        * [Метод checkChainsAggregation](#метод-checkChainsAggregation)
        * [Метод collectExprChains](#метод-collectExprChains)
        * [Метод getUnionHandler](#метод-getUnionHandler)
        * [Метод getRegisteredChain](#метод-getRegisteredChain)
        * [Метод query](#метод-query)
        * [Метод fetchDataModificationCallback](#метод-fetchDataModificationCallback)
        * [Метод isFetchModificationRequired](#метод-isFetchModificationRequired)
        * [Метод replaceSelectAliases](#метод-replaceSelectAliases)
        * [Метод quoteTableSource](#метод-quoteTableSource)
        * [Метод hasBackReference](#метод-hasBackReference)
        * [Метод getChains](#метод-getChains)
        * [Метод getGroupChains](#метод-getGroupChains)
        * [Метод getHiddenChains](#метод-getHiddenChains)
        * [Метод getHavingChains](#метод-getHavingChains)
        * [Метод getFilterChains](#метод-getFilterChains)
        * [Метод getOrderChains](#метод-getOrderChains)
        * [Метод getSelectChains](#метод-getSelectChains)
        * [Метод getWhereChains](#метод-getWhereChains)
        * [Метод getRuntimeChains](#метод-getRuntimeChains)
        * [Метод getQuery](#метод-getQuery)
        * [Метод getLastQuery](#метод-getLastQuery)
        * [Метод buildFilterSql](#метод-buildFilterSql)
        * [Метод getInitAlias](#метод-getInitAlias)
        * [Метод cacheJoins](#метод-cacheJoins)
